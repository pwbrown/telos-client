            ...
            if(config.cb){
                ...
                var responseListener = function(data){
                    ...
                    if(errorListener !== null)
                        this.off(errorKey, errorListener);
                    return resolve(data);
                }.bind(this)
...
function getPropArgValue(method, args, config){
    ...
    if(tc('Undefined', args[pos]))
        return checkForDefault.call(this, `Missing argument '${name}' at position ${pos} of the method '${method}'`, config);
    ...
    if(tc('Array', config.opts) && config.opts.length > 0 && config.opts.indexOf(argValue) === -1)
        return checkForDefault.call(this, `Invalid argument for '${name}' at position ${pos} -> Got '${argValue}' but expected it to be '${config.opts.join(" OR ")}'`, config);
